...
fish_filt = read.csv("fish_sp_filtered.txt",header=T,dec=".",sep="\t", check.names = FALSE) #Fish species with available trait data
...
#prob_occur = prob_occur %>% select(-aquaculture, -ornamental, -lessepsian, -pathway, -status)
...
aquacult = read.csv("Aquaculture_fish.txt",header=T,dec=".",sep="\t", check.names = FALSE) #Fish species with available trait data
...
prob_occur = prob_occur %>% mutate(pathway = case_when(Valid_name %in% lessepsian ~ "lessepsian",
                                                       Valid_name %in% aquacult ~ "aquaculture",
...
inter_g = prob_occur %>% select(Valid_name, pathway) %>% distinct(.); table(inter_g$pathway)
non_def = prob_occur %>% filter(aquaculture == "NO" & ornamental == "NO" & lessepsian == "NO") %>% select(Valid_name) %>% distinct(.) %>% pull(.)
less = prob_occur %>% filter(pathway == "lessepsian") %>% select(Valid_name) %>% distinct(.) %>% pull(.)

prob_occur %>% filter(Valid_name %in% setdiff(lessepsian, less)) %>% select(Valid_name, pathway) %>% distinct(.)
