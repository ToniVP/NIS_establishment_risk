...
#Obtain the map
  map_sum <- ggplot() + theme_map +
    ...
    geom_sf(data = realm, size=1, fill = NA, color = "gray30", linetype=1, alpha = 1) +
    geom_sf(data = worldMap, fill = NA, colour = "saddlebrown", size = 0.1) + 
    geom_sf(data = worldMap, fill = "oldlace", colour = NA, size = 0) + 
...
#Join both plots
Fig1 = ggarrange(map_sum, lat_risk, widths = c(7.6,1), common.legend = T, legend = "bottom",
                 labels = c( "A", "B"), ncol = 2, nrow = 1);
# ggsave(file="./Plots/Fig1.svg", plot=Fig1, width=25, height=15)
# ggsave(file="./Plots/Fig1.png", plot=Fig1, width=30, height=15, dpi = 600)
# ggsave(file="./Plots/Fig1.pdf", plot=Fig1, width=30, height=15, dpi = 600)
...
# ggsave(file="./Plots/Fig2.png", plot=risk_realm, width=15, height=10, dpi = 600)
# ggsave(file="./Plots/Fig2.pdf", plot=risk_realm, width=15, height=10, dpi = 600)
...
# ggsave(file="./Plots/Fig3.pdf", plot=Fig3, width=10, height=6, dpi = 600)
...
# ggsave(file="./Plots/Fig4.pdf", plot=Fig4, width=12.5, height=20, dpi = 600)
...
# Fig S1A
prod_risk_map <- ggplot() + theme_map +
  scale_fill_gradientn(colours=rev(brewer.pal(5, "Spectral")),
                       values= standr(getJenksBreaks(NIS_metrics_global[,"risk_prod"], k = 5)), 
                       #breaks= c(0, 0.25, 0.5, 0.75, 1), 
                       na.value = "aliceblue") +
  geom_tile(data = cells_in_realm, aes(x=CenterLong, y=CenterLat), color = "#2B83BA", fill = "#2B83BA") +
  geom_tile(data = NIS_metrics_global, aes(x=CenterLong, y=CenterLat, fill = NIS_metrics_global[,"risk_prod"])) + #risk
  geom_sf(data = realm, size=1, fill = NA, color = "gray30", linetype=1, alpha = 1) +
  geom_sf(data = worldMap, fill = NA, colour = "saddlebrown", size = 0.1) + 
  geom_sf(data = worldMap, fill = "oldlace", colour = NA, size = 0) + 
  scale_color_manual(values = realm_colors) + scale_y_continuous(expand = c(0, 0)) + scale_x_continuous(expand = c(0, 0)) + 
  guides(color = "none", fill = guide_colorbar(title = "Aggregated invasion risk  ", title.position = "left", title.vjust = 0.7)) + 
  labs(x = "Lon",y="Lat"); prod_risk_map
...
FigS1 = ggarrange(prod_risk_map, mean_risk_map, sd_risk_map, ncol = 1, nrow = 3, common.legend = T, legend = "bottom",
                  labels = c("A", "B", "C")); #FigS1
# ggsave(file="./Plots/FigS1.svg", plot=FigS1, width=12.5, height=10)
# ggsave(file="./Plots/FigS1.pdf", plot=FigS1, width=12.5, height=15, dpi = 600)
...
# ggsave(file="./Plots/FigS2.pdf", plot=FigS2, width=12.5, height=15, dpi = 600)
...
# ggsave(file="./Plots/Fig. S3.svg", plot=FigS3, width=18, height=14)
# ggsave(file="./Plots/FigS3.pdf", plot=FigS3, width=10, height=8, dpi = 600)
